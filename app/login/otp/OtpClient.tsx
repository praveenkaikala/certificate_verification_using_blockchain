...
export default function OTPAuthClient() {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axiosApi({
        ...
        data: { email, role, otp: enteredOtp },
      });
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      <Card className="w-full max-w-md p-6 shadow-md border rounded-xl">
          ...
          <div className="flex justify-between gap-2">
            {otp.map((digit, index) => (
              <input
                ...
                className="w-12 h-12 text-center text-lg font-semibold border rounded-md focus:ring-2 focus:ring-primary/60"
              />
            ))}
          </div>
